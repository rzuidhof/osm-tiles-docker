...
startdb () {
    _startservice /etc/sv/postgresql
}
...
initdb () {
    ...
    then
        echo "Initialisation failed: the directory is not empty: /var/lib/postgresql/9.5/main"
        die "exists"
    fi
...
import () {
    ...
    $asweb psql -d gis -f /home/openstreetmap-carto/drop_indexes.sql
